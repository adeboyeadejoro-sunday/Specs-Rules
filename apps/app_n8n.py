...
# -----------------------------
SHEET_ID = "1s7pETUSVOnvqTZe8BX2noq4ebDJRPC-fY7LzCdhKNDM"

@st.cache_data(ttl=60)
def load_gsheet_data(sheet_id: str) -> pd.DataFrame:
...
def build_active_rules(ps: ParamSpec, spec_id: int) -> List[Dict[str, Any]]:
    ...
    if t == 0:
        perfect = make_rule(base_data(parametertype_id=pid, spec_id=spec_id, unit=unit, target=t, ddf_type="perfect", color="green", operator="<=", value=0.0))
        not_ok = make_rule(base_data(parametertype_id=pid, spec_id=spec_id, unit=unit, target=t, ddf_type="not OK", color="red", operator=">", value=0.0))
        return [perfect, not_ok]

    b = compute_active_bands(t)
    perfect = make_rule(base_data(parametertype_id=pid, spec_id=spec_id, unit=unit, target=t, ddf_type="perfect", color="green", operator=">=", operator2="<=", linker="AND", value=b.low_perfect, value2=b.high_perfect))
    ok_low = make_rule(base_data(parametertype_id=pid, spec_id=spec_id, unit=unit, target=t, ddf_type="OK", color="orange", operator=">=", operator2="<", linker="AND", value=b.low_ok, value2=b.low_perfect))
    ok_high = make_rule(base_data(parametertype_id=pid, spec_id=spec_id, unit=unit, target=t, ddf_type="OK", color="orange", operator=">", operator2="<=", linker="AND", value=b.high_perfect, value2=b.high_ok2))
    not_ok = make_rule(base_data(parametertype_id=pid, spec_id=spec_id, unit=unit, target=t, ddf_type="not OK", color="red", operator="<", operator2=">", linker="OR", value=b.low_ok, value2=b.high_ok2))
...
def build_limit3_rules(ps: ParamSpec, spec_id: int) -> List[Dict[str, Any]]:
    ...
    perfect = make_rule(base_data(parametertype_id=pid, spec_id=spec_id, unit=unit, target=t, ddf_type="perfect", color="green", operator="<=", value=threshold_perfect))
    ok = make_rule(base_data(parametertype_id=pid, spec_id=spec_id, unit=unit, target=t, ddf_type="OK", color="orange", operator=">", operator2="<=", linker="AND", value=threshold_perfect, value2=r2(t)))
...
def build_limit2_rules(ps: ParamSpec, spec_id: int) -> List[Dict[str, Any]]:
    ...
    t = float(ps.target)
    unit = ps.unit
    perfect = make_rule(base_data(parametertype_id=pid, spec_id=spec_id, unit=unit, target=t, ddf_type="perfect", color="green", operator="<=", value=r2(t)))
    not_ok = make_rule(base_data(parametertype_id=pid, spec_id=spec_id, unit=unit, target=t, ddf_type="not OK", color="red", operator=">", value=r2(t)))
    return [perfect, not_ok]
...
def build_qualitative_rules(ps: ParamSpec, spec_id: int, qual_en: str, qual_de: str) -> List[Dict[str, Any]]:
    ...
    t = float(ps.target)
    unit = ps.unit
    perfect = make_rule(base_data(parametertype_id=pid, spec_id=spec_id, unit=unit, target=t, ddf_type="perfect", color="green", operator="=", operator2="=", linker="OR", value=qual_en, value2=qual_de))
    not_ok = make_rule(base_data(parametertype_id=pid, spec_id=spec_id, unit=unit, target=t, ddf_type="not OK", color="red", operator=">", value=r2(t)))
    return [perfect, not_ok]
...
def build_rules_from_specs(spec_id: int, param_specs: List[ParamSpec], qual_en: str, qual_de: str) -> List[Dict[str, Any]]:
    ...
    for ps in param_specs:
        if ps.mode == "active":
            if ps.target is None: raise ValueError("Active requires numeric target.")
            rules = build_active_rules(ps, spec_id)
        elif ps.mode == "mineral":
            if ps.target is None: raise ValueError("Mineral requires numeric target.")
            rules = build_mineral_rules(ps, spec_id)
        elif ps.mode == "limit3":
            if ps.target is None: raise ValueError("Limit3 requires numeric target.")
            rules = build_limit3_rules(ps, spec_id)
        elif ps.mode == "limit2":
            if ps.target is None: raise ValueError("Limit2 requires numeric target.")
...
def perform_clear_all_pre_widgets() -> None:
    ...
    st.session_state["rows_initialized"] = False # Forces reload from GSheet if URL params exist
...
if "qual_de" not in st.session_state:
    st.session_state["qual_de"] = ""
if "spec_options" not in st.session_state:
    st.session_state["spec_options"] = [0]
if "rows_initialized" not in st.session_state:
    st.session_state["rows_initialized"] = False


# Automatic Initialization from URL & GSheet
query_params = st.query_params
sample_id_target = query_params.get("sample")

if sample_id_target and not st.session_state["rows_initialized"]:
    try:
        df = load_gsheet_data(SHEET_ID)
        sample_df = df[df['Sample ID'].astype(str) == sample_id_target]
        ...
        if not sample_df.empty:
            ...
            for _, row in sample_df.iterrows():
                ...
                mode = "mineral" if category.lower() == "mineral" else "active"
                
                initial_rows.append({
                    "parametertype_id": int(row['Parameter ID']),
                    "mode": mode,
                    "target_is_null": False,
                    "target_value": 0.0,
                    "unit_is_null": False,
